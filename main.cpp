...
const double CHANGE_IN_IMAGE_FOR_COLLECTION = 0.3;      // How much the facial image sho r uld change before collecting a new face photo for training.
...
void onMouse(int event, int x, int y, int, void*)
{
	...
	cin.getline(s.name, 51);            
        s.roll = m_numPersons;
        int r = s.roll;
	f.seekp(((r)* sizeof(student)),ios::beg);
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
	...
	*/
	...
	if (similarity > 0.4)
	{
		...
		if (k != 0 && k % ACCURACY == 0)
		{
			t.open("test.txt",ios::in|ios::out|ios::trunc);
			a.open("accuracy.txt", ios::out|ios::in);
			f.open("kv.dat",ios::binary|ios::in|ios::out);
			int counter[2*m_numPersons], max = 0, temp = 0;
			for(int i = 0; i < 2*m_numPersons; ++i)
			{
				counter[i] = 0;
			}
			while( !a.eof() )
			...
			student s;
			int r = max;
			f.seekg(((r)* sizeof(student)), ios::beg);
			f.read((char*)&s, sizeof(student));
			if (s.roll == -1 && s.name[0] == '\0')
			{
				cerr << "\n***Not Found***\n";
				cerr << "\nPress <enter> "; cin.get();
			}
			...
			{
				int j;
				for(j = 0; j < strlen(s.name)-1; ++j)
					t << s.name[j] << ',';
				t << s.name[j];
			
				cout << s.name << "\tIdentity: " << max << ". Similarity: " << similarity << endl;
				t.close();
				sleep(20);
			}
			a.close();
		...
		else
		{
			if(similarity < UNKNOWN_PERSON_THRESHOLD)
			{
				a.open("accuracy.txt",ios::out|ios::in|ios::app);
				a << identity << endl;
				a.close();
				k += 1;	
			}
